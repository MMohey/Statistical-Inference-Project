---
title: "Statistical Inference Project_Part 1"
author: "Moh Ahmed"
date: "04/02/2017"
...
1. Set the seed to an arbitrary value for reproducability.
...
ggplot(exp_dist_mn, aes(mn)) + geom_histogram() + geom_vline(aes(xintercept=value, color=type),data=mns_mat, lwd = 1, show.legend = T) + 
    scale_color_manual("Mean type",
                       values=c("Theoretical"="yellow","Sample"="red"))
```
...
```{r, warning=FALSE}
 ggplot(exp_dist_mn, aes(mn)) + geom_histogram( aes(y= ..density..),alpha = 0.5) + geom_vline(aes(xintercept=value, color=type),data=mns_mat, lwd = 1, show.legend = T) +    scale_color_manual("Mean type", values=c("Theoretical"="yellow","Sample"="red")) + stat_function(fun = dnorm, args = list(mean = theoretical_mn, sd = sqrt(theoretical_var)), colour = 'yellow', size=1) + geom_density(color = 'red', size =1) 
```
